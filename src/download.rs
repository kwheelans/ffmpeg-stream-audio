...
pub async fn download_css_archive() -> Result<(), Error> {
    info!("Downloading Pico CSS from {}", PICO_CSS_URL);
    let response = reqwest::get(PICO_CSS_URL).await?.error_for_status()?;
    ...
    let extract_path = PathBuf::from(PICO_CSS_PATH);
